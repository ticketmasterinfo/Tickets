...
/**
 * Official Ticketmaster Brand Logo
 * Renders the authentic vector wordmark exactly as seen on the homepage navbar.
 */
export const TicketmasterLogo: React.FC<TicketmasterLogoProps> = ({
  ...
  return (
    ...
    >
      ...
      <path d={TICKETMASTER_SVG_PATH} />
    </svg>
  );
};

/**
 * Signature 't' icon emblem
 */
export const TicketmasterEmblem: React.FC<{ className?: string; color?: string; size?: number }> = ({
  className = "w-5 h-5",
  color = "#ffffff",
  size
}) => {
  return (
    <svg 
      ...
      width={size}
      height={size}
      aria-label="Ticketmaster Emblem"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d={TICKETMASTER_T_PATH} />
    </svg>
  );
};

/**
 * Generates an authentic ultra-high-resolution PNG Data URL of the Ticketmaster wordmark
 * for direct, crystal-clear vector rasterization into jsPDF or print documents.
 */
export function getTicketmasterLogoDataUrl(fillColor: string = '#ffffff', scale: number = 8): string {
  try {
    if (typeof document === 'undefined') return '';
    const canvas = document.createElement('canvas');
    canvas.width = Math.round(135 * scale);
    canvas.height = Math.round(24 * scale);
    ...
    if (typeof Path2D !== 'undefined') {
      const path = new Path2D(TICKETMASTER_SVG_PATH);
      ctx.fill(path);
      return canvas.toDataURL('image/png');
    }
  } catch (e) {
    console.warn('Canvas Path2D rasterization unavailable, falling back to SVG URI', e);
  }

  // Fallback: encode clean SVG data URL
  const svgString = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 135 24" fill="${fillColor}"><path d="${TICKETMASTER_SVG_PATH}"/></svg>`;
