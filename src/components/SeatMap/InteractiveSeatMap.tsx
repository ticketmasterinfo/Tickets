...
import { getEventSeatZones } from '../../data/eventsData';
...
export const InteractiveSeatMap: React.FC<InteractiveSeatMapProps> = ({
  ...
  const zones = getEventSeatZones(event);
    ...
    <div className="relative bg-[#1a1a1a] rounded-2xl border border-gray-800 overflow-hidden shadow-2xl p-4 md:p-6 text-white min-h-[560px] flex flex-col justify-between">
      ...
      <div className="relative flex-1 flex items-center justify-center overflow-hidden py-4 my-2">
        ...
        >
          {!isSectionDetailedView ? (
            ...
            <div className="bg-[#121212] p-4 sm:p-6 rounded-xl border border-gray-700 shadow-2xl max-w-lg w-full">
              ...
              {/* Seat Matrix */}
              <div className="space-y-2 py-2 overflow-x-auto pb-1">
                {['A', 'B', 'C', 'D', 'E'].map((row) => (
      ...
      <div className="pt-3 border-t border-gray-800 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-300">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {zones.map(z => (
            <div 
              ...
              title={`${z.name} - ${z.availableCount} tickets available`}
            >
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: z.color }}></span>
              <span className="text-[11px] font-medium">{z.code} (${z.price})</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center space-x-1 text-gray-400 text-[11px]">
          ...
          <span>Click any zone or tier to select exact seats</span>
        </div>
      </div>
