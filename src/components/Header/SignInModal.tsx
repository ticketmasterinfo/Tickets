...
interface SignInModalProps {
  ...
  onSuccessSignIn?: (email: string, name: string) => void;
}

export const SignInModal: React.FC<SignInModalProps> = ({
  isOpen,
  onClose,
  currentUserEmail = 'ticketmastersincinfo@gmail.com',
  onSuccessSignIn
}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (onSuccessSignIn) {
        onSuccessSignIn(email, fullName);
      }
      onClose();
    }, 600);
  };
  ...
  return (
    ...
    >
      <div 
        className="bg-white text-[#121212] w-full max-w-md rounded-2xl shadow-2xl border border-gray-200 overflow-hidden relative"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Top Header with TM Blue Accent */}
        <div className="bg-[#024ddf] px-6 py-5 text-white flex items-center justify-between relative">
          <div className="flex items-center space-x-3">
            {/* Ticketmaster Logo Vector */}
            <svg 
            ...
            <span className="text-xs bg-white/20 px-2 py-0.5 rounded font-semibold tracking-wide uppercase">
              Official Account
            </span>
          </div>
          <button 
        ...
        <div className="flex border-b border-gray-200">
          ...
          >
            New to Ticketmaster? Register
          </button>
        </div>

        {/* Modal Body / Form */}
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <h2 id="signin-modal-title" className="text-xl font-extrabold text-[#121212]">
            {mode === 'signin' ? 'Sign In to Your Account' : 'Create Your Ticketmaster Account'}
          </h2>
          <p className="text-xs text-gray-600">
            {mode === 'signin'
              ? 'Access your tickets, manage orders, and enjoy faster checkout.'
              : 'Sign up to get access to presales, live alerts, and mobile ticketing.'}
          </p>
          ...
          {mode === 'register' && (
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-[#024ddf] focus:border-transparent"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              {mode === 'signin' && (
                <button
                  type="button"
                  onClick={() => alert('A password reset link has been dispatched to ' + email)}
                  className="text-xs font-semibold text-[#024ddf] hover:underline"
                >
                  Forgot Password?
                </button>
              )}
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-[#024ddf] focus:border-transparent pr-10"
                placeholder="••••••••••••"
              />
              <button
                type="button"
          ...
          <div className="flex items-center justify-between pt-1">
            <label className="flex items-center space-x-2 text-xs text-gray-700 cursor-pointer">
              <input 
                ...
                className="rounded border-gray-300 text-[#024ddf] focus:ring-[#024ddf]"
              />
          ...
          <button
            type="submit"
            disabled={submitted}
            className="w-full py-3 bg-[#024ddf] hover:bg-[#0139a7] active:bg-[#012e85] text-white font-bold rounded-lg transition-colors duration-200 text-sm flex items-center justify-center space-x-2 shadow-sm cursor-pointer"
          >
            {submitted ? (
              <>
          ...
          <div className="pt-3 border-t border-gray-100 text-center">
            <div className="flex items-center justify-center space-x-1.5 text-gray-500 text-[11px]">
              ...
              <span>Secured by Ticketmaster SafeTix™ & 256-bit encryption</span>
            </div>
          </div>
        </form>
      </div>
